...
- (void)viewDidLoad {
	...
	// View
	self.view.backgroundColor = [UIColor colorWithRed:146.0f/255.0f green:90.0f/255.0f blue:202.0f/255.0f alpha:1.0f];
    ...
    if ([_toolbar respondsToSelector:@selector(setBarTintColor:)]) {
        _toolbar.barTintColor = [UIColor colorWithRed:146.0f/255.0f green:90.0f/255.0f blue:202.0f/255.0f alpha:1.0f];
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        if (_uploadButton) {
            _uploadBut =[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Upload", nil) style:UIBarButtonItemStylePlain target:self action:@selector(uploadButtonPressed:)];
            self.navigationItem.rightBarButtonItem = _uploadBut;
        }
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if ([self.navigationController.viewControllers indexOfObject:self] == NSNotFound) {
        ...
        if([self.delegate respondsToSelector:@selector(photoBrowser:backDelegate:)]){
            [self.delegate photoBrowser:self backDelegate:0];
        }
    }
...
- (void)showGrid:(BOOL)animated {
    ...
    _gridController.desText = _desText;
...
- (void)hideGrid {
    ...
    [UIView animateWithDuration:0.3
                        ...
                        options: UIViewAnimationCurveEaseInOut
                     animations:^{
                         //tmpGridController.view.alpha = 1;
...
- (void)doneButtonPressed:(id)sender {
    if ([self.delegate respondsToSelector:@selector(photoBrowser:DoneTappedDelegate:)]) {
        [self.delegate photoBrowser:self DoneTappedDelegate:0];
    }
    [self dismissViewControllerAnimated:YES completion:nil];
}

- (void)uploadButtonPressed:(id)sender {
    if ([self.delegate respondsToSelector:@selector(photoBrowser:uploadTappedDelegate:)]) {
        [self.delegate photoBrowser:self uploadTappedDelegate:0];
    }
}
